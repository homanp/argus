...
function readPersistedWidth(): number {
  try {
    const match = document.cookie.match(new RegExp(`(?:^|; )${SIDEBAR_COOKIE_NAME}=(\\d+)`))
    if (match) return Math.min(SIDEBAR_MAX_WIDTH, Math.max(SIDEBAR_MIN_WIDTH, Number(match[1])))
  } catch {
    // noop
  }
...
function SidebarProvider({ className, style, children, ...props }: React.ComponentProps<"div">) {
  ...
  const [sidebarWidth, _setSidebarWidth] = React.useState(readPersistedWidth)
  ...
  const setSidebarWidth = React.useCallback((w: number) => {
    ...
    document.cookie = `${SIDEBAR_COOKIE_NAME}=${clamped}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`
  }, [])
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    ...
    [sidebarWidth, setSidebarWidth, isResizing, setIsResizing, isMobile, openMobile, setOpenMobile],
  )
...
function SidebarResizeHandle({ className, ...props }: React.ComponentProps<"div">) {
  const { sidebarWidth, setSidebarWidth, setIsResizing } = useSidebar()
